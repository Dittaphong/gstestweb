      ...
      <div class="form-horizontal">
        <div class="form-group">
          <div class="col-md-12 col-input">
            ...
            <?php else : ?>
            <?php foreach ($Result['PROGRAM_STRUC'] as $PROGRAM_STRUC): ?>
            <div class="col-md-6"> <?php echo form_open('graduate/SaveStructure/'); ?>
              <input type="text" class="form-control" placeholder="แผนการศึกษา"
			name="PROGRAMPLAN"
			value="<?php $this->debuger->NullObj($PROGRAM_STRUC['PROGRAMPLAN']); ?>">
            </div>
            <div class="col-md-3">
              <input type="text" class="form-control" placeholder="หน่วยกิต"
			name="MAXCREDIT"
			value="<?php $this->debuger->NullObj($PROGRAM_STRUC['MAXCREDIT']); ?>">
            </div>
            <div class="col-md-3"> 
              <!-- btn save prog structure-->
              
              <input type="hidden" name="PROGRAMID" value="<?php echo $Result['PROGRAM'][0]['PROGRAMID']; ?>">
              <input type="hidden" name="PROGRAMSTRUCTUREID" value="<?php echo $PROGRAM_STRUC['PROGRAMSTRUCTUREID']; ?>">
              <button type="submit" class="btn btn-success"> <i class="fa fa-save"></i> </button>
              <?php echo form_close(); ?> 
              <!-- btn del prog structure-->
              <button type="button" class="btn btn-danger"
                    onClick="window.location.href='<?php echo site_url(); ?>/graduate/DelStructure/<?php echo $PROGRAM_STRUC['PROGRAMSTRUCTUREID']; ?>/<?php echo $Result['PROGRAM'][0]['PROGRAMID']; ?>'"> <i class="glyphicon glyphicon-remove"></i> </button>
            </div>
            <?php endforeach; ?>
