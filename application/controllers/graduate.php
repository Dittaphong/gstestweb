...
class graduate extends CI_Controller {
	...
	public function index(){ 	//หน้า Dashboard
		$StrQuery = array(
			'Result' => $this->graduatemodel->student_all(),
			'View' => 'Graduate/Pages/Dashboard'
		);
	...
	public function AdvisorList(){ // รายการ รออนุมัติที่ปรึกษา
		...
		$StrQuery = array(
			'Result' => $this->graduatemodel->AdvisorList(),
			'View' => 'Graduate/Pages/Students/AdvisorList'
		);
		$this->loadpage($StrQuery);
	}
	public function ExamList(){ // รายการ รออนุมัติสอบ QE/CE
		$EXAM_TYPE = $this->uri->segment(3);
		$ExamList = $this->graduatemodel->ExamList($EXAM_TYPE);
		$StrQuery = array(
			'Result' => $ExamList,
			'View' => 'Graduate/Pages/Students/ExamList'
		);
		$this->loadpage($StrQuery);
	}
	public function PropersalList(){ // รายการ รออนุมัติสอบ เค้าโครง
		$StrQuery = array(
			'Result' => $this->graduatemodel->PropersalList(),
			'View' => 'Graduate/Pages/Students/PropersalList'
		);
		$this->loadpage($StrQuery);
	}
	public function ThesisList(){ // รายการ รออนุมัติสอบ วิทยานิพนธ์
		$StrQuery = array(
			'Result' => $this->graduatemodel->ThesisList(),
			'View' => 'Graduate/Pages/Students/ThesisList'
		);
		$this->loadpage($StrQuery);
	}
	...
	public function StudentCommand(){ //ข้อมูลนักศึกษา และจัดการได้ (ใช้ร่วมกันกับการจัดการเอกสารทั้งหมด)
		...
		$relate_adviser = $this->db->where('doc_approved_type', 'ADVI')
		...
		// $this->debuger->prevalue($doc_exam);
		...
		$StrQuery = array(
				...
				// ไฟล์หน้าเว็บ
				'View' => 'Graduate/Pages/Students/StudentCommand'
			);
 ...
 // exit();
	...
	// คำสั่งจัดการ เอกสาร
	public function AdvisorApproved(){ // อนุมัติอาจารย์ที่ปรึกษา
		$input = array(
						'APPOINT_AVISER_IS_THESIS_ID' => $this->uri->segment(3),
						'DOCUMENTSTATUS' => 1);
		$this->docsmodel->adviser($input);
		redirect($this->agent->referrer(), 'refresh');
	}
	...
	public function OfficerList(){
		$StrQuery = array(
			...
			'View' => 'Graduate/Pages/Officers/OfficerList' // View
		);
		$this->loadpage($StrQuery);
	}
	// ข้อมูลอาจารย์
	public function Officerinfo(){
		...
		$StrQuery = array(
			...
			'View' => 'Graduate/Pages/Officers/OfficerInfo'
		);
	...
	public function ProgramsSearch(){
		...
		$StrQuery = array(
			...
			'View' => 'Graduate/Pages/Programs/ManagePrograms'
		);
		$this->loadpage($StrQuery);
	}
	//ผลการค้นหา
	public function ProgramsResult(){
		...
		$StrQuery = array(
			'Result' => $allprog,
			'namepage' =>'ข้อมูลหลักสูตร' ,
			'View' => 'Graduate/Pages/Programs/ManageProgramsbyFilter');
	...
	public function FormEditProgram() {
		...
		$StrQuery = array(
			...
			'View' => 'Graduate/Pages/Programs/FormAddProgram');
  ...
  // บันทึกโครงสร้างหลักสูตร
    ...
    $query = $this->db->get('doc_adviser');
		...
		$StrQuery = array(
			...
			'View' => 'Graduate/Pages/Students/ApprovedAdvisorList'
		);
		$this->loadpage($StrQuery);
