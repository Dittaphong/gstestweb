...
class facmodel extends CI_Model {
	...
	public function DocAdviserList(){
	...
	public function DocAdviserStatus($value){
		$this->db->where('doc_approved_ref', $value);
		$this->db->where('doc_approved_type', 'ADVI');
		$this->db->where('doc_approved_status', 1);
		$this->db->where('doc_approved_by','DEAN');
		return $query = $this->db->get('relate_doc_approved')->num_rows();
	}
	...
	public function Proposal($input){
		$this->db->where('STUDENTID', $input);
		$query = $this->db->get('doc_exam_proposal');
		return $query->result_array();
	}
	public function Thesis($input){
		$this->db->where('STUDENTID', $input);
		$query = $this->db->get('doc_exam_thesis');
		return $query->result_array();
	}
	...
	public function ExamList($EXAM_TYPE){
		$this->db->where('doc_approved_type', $EXAM_TYPE);
		$this->db->where('doc_approved_by', 'FACU');
		$this->db->where('doc_approved_status', 0);
		$this->db->where('EXAM_TYPE',$EXAM_TYPE);
		if ($EXAM_TYPE=='ADVI') {
			$this->db->join('doc_adviser', 'relate_doc_approved.doc_approved_ref = doc_exam.EXAM_ID','LEFT');
				$this->db->join('relate_doc_approved', 'doc_adviser.APPOINT_AVISER_ID = relate_doc_approved.doc_approved_ref');
				echo "string";
				exit();
		}
		...
		$query = $this->db->get('relate_doc_approved');
		return $query->result_array();
	}
